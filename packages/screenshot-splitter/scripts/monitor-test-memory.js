...
class MemoryMonitor {
  ...
  start() {
    ...
    this.interval = setInterval(() => {
      ...
      // 实时显示
      console.log(
        `[${new Date().toLocaleTimeString()}] 系统内存: ${formatMemory(systemMem.used)}/${formatMemory(systemMem.total)} (${systemMem.usagePercent.toFixed(1)}%) | 进程内存: ${formatMemory(processMem.rss)}`
      );
      ...
      if (processMem.rss > 2 * 1024 * 1024 * 1024) {
        // 2GB
  ...
  generateReport() {
    ...
    if (this.samples.length > 0) {
      const avgSystemUsage =
        this.samples.reduce((sum, sample) => sum + sample.system.used, 0) / this.samples.length;
      const avgProcessUsage =
        this.samples.reduce((sum, sample) => sum + sample.process.rss, 0) / this.samples.length;
    ...
    // 内存使用建议
    console.log('\n💡 优化建议:');
    if (this.maxProcessMemory > 1.5 * 1024 * 1024 * 1024) {
      // 1.5GB
