...
export const useDebugState = ({
  ...
  const getDiagnostics = () => {
    ...
    if (
      debugSnapshot.stateDetails &&
      debugSnapshot.slicesCount !== debugSnapshot.stateDetails.imageSlicesLength
    ) {
    ...
    return {
      ...
      recommendations:
        issues.length > 0
          ? ['检查ImagePreview组件的props传递', '验证切片URL的生成逻辑', '确认状态更新的时序']
          : ['状态正常'],
    };
  };
