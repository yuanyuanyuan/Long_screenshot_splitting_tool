...
export function useImageProcessor({ state, actions }: UseImageProcessorProps): ImageProcessorHookReturn {
  ...
  const handleChunk = useCallback((blob: Blob, index: number) => {
    ...
    img.onload = () => {
      ...
      console.log('[ImageProcessor] 添加图片切片到状态:', index, img.naturalWidth, img.naturalHeight);
      const imageSlice = {
        blob,
        url,
        index,
