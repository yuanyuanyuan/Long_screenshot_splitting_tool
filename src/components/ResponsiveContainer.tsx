...
export const ResponsiveContainer: React.FC<ResponsiveContainerProps> = ({
  ...
  const touchClasses =
    enableTouchOptimization && viewport.isTouch ? 'touch-manipulation tap-highlight-none' : '';

  // Combine all classes
  const finalClasses = [baseClasses, breakpointClasses, touchClasses, className]
    .filter(Boolean)
    .join(' ');

  const ElementComponent = Component as ElementType;

  return <ElementComponent className={finalClasses}>{children}</ElementComponent>;
};
...
export const ResponsiveFlexContainer: React.FC<{
...
}> = ({ children, className = '', direction = 'responsive' }) => {
  const viewport = useViewport();
  ...
  const gapClasses = viewport.isMobile ? 'gap-3' : viewport.isTablet ? 'gap-4' : 'gap-6';

  return (
    <div className={`${directionClasses[direction]} ${gapClasses} ${className}`}>{children}</div>
  );
};
...
export const ResponsiveGridContainer: React.FC<{
  ...
  return <div className={`${gridClasses} ${gapClasses} ${className}`}>{children}</div>;
};
...
export const ResponsiveButtonContainer: React.FC<{
...
}> = ({ children, className = '', layout = 'responsive' }) => {
  const viewport = useViewport();
  ...
  return (
    <div className={`${layoutClasses[layout]} ${spacingClasses} ${className}`}>{children}</div>
  );
};
...
export const SafeAreaContainer: React.FC<{
  ...
  const paddingClasses = includePadding ? (viewport.isMobile ? 'px-4 py-2' : 'px-6 py-4') : '';

  const safeAreaClasses = 'safe-area-inset-top safe-area-inset-bottom';

  return <div className={`${paddingClasses} ${safeAreaClasses} ${className}`}>{children}</div>;
};
