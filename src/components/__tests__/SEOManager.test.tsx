...
describe('SEOManager', () => {
  ...
  describe('元数据生成', () => {
    it('应该使用正确的参数调用元数据生成器', () => {
      ...
      render(<SEOManager {...defaultProps} page="split" context={context} language="en" />);
    ...
    it('应该合并自定义元数据', () => {
      ...
      render(<SEOManager {...defaultProps} customMetadata={customMetadata} />);
    ...
    it('应该处理空的自定义元数据', () => {
      render(<SEOManager {...defaultProps} customMetadata={{}} />);
  ...
  describe('功能开关', () => {
    it('应该根据enableStructuredData控制结构化数据', () => {
      const { rerender } = render(<SEOManager {...defaultProps} enableStructuredData={false} />);

      let helmet = screen.getByTestId('helmet');
      expect(helmet).toBeInTheDocument();

      rerender(<SEOManager {...defaultProps} enableStructuredData={true} />);
    ...
    it('应该根据enableOpenGraph控制OG标签', () => {
      const { rerender } = render(<SEOManager {...defaultProps} enableOpenGraph={false} />);

      let helmet = screen.getByTestId('helmet');
      expect(helmet).toBeInTheDocument();

      rerender(<SEOManager {...defaultProps} enableOpenGraph={true} />);
    ...
    it('应该根据enableTwitterCard控制Twitter卡片', () => {
      const { rerender } = render(<SEOManager {...defaultProps} enableTwitterCard={false} />);

      let helmet = screen.getByTestId('helmet');
      expect(helmet).toBeInTheDocument();

      rerender(<SEOManager {...defaultProps} enableTwitterCard={true} />);
    ...
    it('应该根据enableCanonical控制规范URL', () => {
      const { rerender } = render(<SEOManager {...defaultProps} enableCanonical={false} />);

      let helmet = screen.getByTestId('helmet');
      expect(helmet).toBeInTheDocument();

      rerender(<SEOManager {...defaultProps} enableCanonical={true} />);
  ...
  describe('结构化数据', () => {
    it('应该为首页生成SoftwareApplication结构化数据', () => {
      render(<SEOManager {...defaultProps} page="home" enableStructuredData={true} />);
    ...
    it('应该为其他页面生成WebApplication结构化数据', () => {
      render(<SEOManager {...defaultProps} page="upload" enableStructuredData={true} />);
  ...
  describe('多语言支持', () => {
    it('应该为中文生成正确的locale', () => {
      render(<SEOManager {...defaultProps} language="zh-CN" enableOpenGraph={true} />);
    ...
    it('应该为英文生成正确的locale', () => {
      render(<SEOManager {...defaultProps} language="en" enableOpenGraph={true} />);
    ...
    it('应该生成hreflang链接', () => {
      render(<SEOManager {...defaultProps} enableCanonical={true} />);
  ...
  describe('性能优化', () => {
    it('应该使用useMemo优化元数据生成', () => {
      ...
      const { rerender } = render(
        <SEOManager {...defaultProps} context={context} customMetadata={customMetadata} />
      );
      ...
      rerender(<SEOManager {...defaultProps} context={context} customMetadata={customMetadata} />);
    ...
    it('应该在props变化时重新生成元数据', () => {
      const { rerender } = render(<SEOManager {...defaultProps} context={{ sliceCount: 5 }} />);

      const mockFn = vi.mocked(metadataGenerator.generatePageMetadata);
      const initialCallCount = mockFn.mock.calls.length;

      // 不同props重新渲染应该重新生成元数据
      rerender(<SEOManager {...defaultProps} context={{ sliceCount: 10 }} />);
  ...
  describe('边界条件', () => {
    it('应该处理undefined的context', () => {
      render(<SEOManager {...defaultProps} context={undefined} />);

      expect(screen.getByTestId('helmet')).toBeInTheDocument();
    });

    it('应该处理空的customMetadata', () => {
      render(<SEOManager {...defaultProps} customMetadata={undefined} />);

      expect(screen.getByTestId('helmet')).toBeInTheDocument();
    });

    it('应该处理无效的页面类型', () => {
      render(<SEOManager {...defaultProps} page={'invalid' as any} />);

      expect(screen.getByTestId('helmet')).toBeInTheDocument();
    });

    it('应该处理无效的语言', () => {
      render(<SEOManager {...defaultProps} language={'invalid' as any} />);
  ...
  describe('集成测试', () => {
    it('应该与元数据生成器正确集成', () => {
      ...
      render(<SEOManager {...defaultProps} page="split" context={context} language="zh-CN" />);

      expect(metadataGenerator.generatePageMetadata).toHaveBeenCalledWith(
        'split',
        context,
        'zh-CN'
      );
  ...
  describe('可访问性', () => {
    it('应该包含正确的语言属性', () => {
      render(<SEOManager {...defaultProps} language="zh-CN" />);
