...
export const useSEOI18n = (page: PageType = 'home') => {
   ...
   */
  const generateContextualKeywords = useCallback(
    (context: KeywordContext): string[] => {
      ...
      if (context.imageType) {
        const imageTypeKeywords = {
          chat:
            language === 'zh-CN' ? ['聊天截图', '对话记录'] : ['chat screenshot', 'conversation'],
          webpage:
            language === 'zh-CN' ? ['网页截图', '长网页'] : ['webpage screenshot', 'long page'],
          document:
            language === 'zh-CN'
              ? ['文档截图', 'PDF截图']
              : ['document screenshot', 'PDF screenshot'],
        };
  ...
  const getFormattedDescription = useCallback(
    (params?: { imageCount?: number; processingTime?: number }): string => {
   ...
   */
  const switchLanguageWithSEO = useCallback(
