 ...
 */
function handleFileSelect(e, appState) {
 ...
 * @param {Object} appState - 应用状态对象
 */
function handleDrop(e, appState) {
  e.preventDefault();
  const dropZone = document.getElementById("dropZone");
  dropZone.classList.remove("drag-over");
...
function loadImage(file, appState) {
  if (file.size > 50 * 1024 * 1024) {
    alert(window.i18n ? window.i18n.t("js.alert.fileTooLarge") : "文件过大");
    return;
  }
...
function processImage(appState) {
  ...
  if (isNaN(sliceHeight) || sliceHeight < 100 || sliceHeight > 5000) {
    alert(window.i18n ? window.i18n.t("js.alert.invalidSliceHeight") : "分割高度无效");
    return;
  }

  // 1. 执行清理函数：清理之前会话的所有资源
  console.log('[FileProcessor] 开始资源清理...');
  window.cleanupPreviousSession(appState);
...
function handleChunkMessage(blob, index, appState) {
  ...
  // 调用预览界面模块的函数创建缩略图
  window.addThumbnailToList(imageUrl, index, appState);
...
function handleProcessingComplete(appState) {
  ...
  // 3. 启用导出按钮（新预览界面的按钮）
  window.toggleExportButtons(appState);
...
window.handleChunkMessage = handleChunkMessage;
window.handleProcessingComplete = handleProcessingComplete;
window.handleProcessingError = handleProcessingError;