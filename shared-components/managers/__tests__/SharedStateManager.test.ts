...
describe('SharedStateManager', () => {
  ...
  describe('状态订阅', () => {
    test('应该能够订阅状态变化', () => {
      ...
      expect(callback).toHaveBeenCalledWith({ name: 'Alice' }, undefined, 'user');
    });
    ...
    test('应该支持多个订阅者', () => {
      ...
      expect(callback1).toHaveBeenCalledWith({ name: 'David' }, undefined, 'user');
      expect(callback2).toHaveBeenCalledWith({ name: 'David' }, undefined, 'user');
    });
  ...
  describe('错误处理', () => {
    ...
    test('应该处理JSON解析错误', () => {
      const mockGetItem = jest.spyOn(Storage.prototype, 'getItem').mockReturnValue('invalid-json');
